...
# X_pca = pca.fit_transform(XFull)

scaler = StandardScaler().fit(XFull)
X_final = scaler.transform(XFull)
...
log_reg = LogisticRegression(max_iter= 20, solver='newton-cholesky', penalty='l2').fit(X_final, y_train)
y_pred_train = log_reg.predict(X_final)

y_pred_test = log_reg.predict(x_test)

print("Logistic regression - accuracy on train data: ", log_reg.score(X_final, y_train))
